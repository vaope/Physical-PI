...
static void usb_mouse_as_key_irq(struct urb *urb)
{
    ...
    printk("data[0] = 0x%x ,  0x%x , 0x%x , 0x%x\n", data[0], data[1], data[2], data[3]);
	input_report_key(dev, BTN_LEFT,   data[0] & 0x01);
	input_report_key(dev, BTN_RIGHT,  data[0] & 0x02);
	input_report_key(dev, BTN_MIDDLE, data[0] & 0x04);
	input_report_key(dev, BTN_SIDE,   data[0] & 0x08);
	input_report_key(dev, BTN_EXTRA,  data[0] & 0x10);
    // input_report_key(dev, BTN_FORWARD, data[0] & 0x20);
	// input_report_key(dev, BTN_BACK, data[0] & 0x40);
	// input_report_key(dev, BTN_TASK, data[0] & 0x80);
...
static int usb_mouse_as_key_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    desc->data_buffer = usb_alloc_coherent(dev, 8, GFP_ATOMIC, &desc->data_dma);
	...
	input_dev->keybit[BIT_WORD(BTN_MOUSE)] |= BIT_MASK(BTN_SIDE) |
		BIT_MASK(BTN_EXTRA);
    input_dev->keybit[BIT_WORD(BTN_MOUSE)] |= BIT_MASK(BTN_FORWARD) |
